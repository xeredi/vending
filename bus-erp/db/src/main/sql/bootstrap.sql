...
CREATE USER transportadm WITH PASSWORD 'transport';
CREATE USER transport WITH PASSWORD 'transport';
CREATE DATABASE transport WITH OWNER transportadm;

CREATE SCHEMA transport;
GRANT USAGE ON SCHEMA transport TO transport;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA transport TO transport;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA transport TO transport;

ALTER ROLE transport SET search_path TO transport, public;
ALTER ROLE transportadm SET search_path TO transport, public;
